...
t_triangle	init_trian(void)
{
	...
	triangle.v0 = (t_vec3f){0.0, 0.0, 0.2};
...
t_sphere	init_sphere(void) // если плоскость пересекает сферу в 0 0 0, то так должно быть
{
	t_sphere sphere;
...
t_cyl init_cyl(void)
{
	...
	cyl.cen = (t_vec3f){0.0, -0.2, 0.0};
	cyl.n = (t_vec3f){0.0, 1.0, -0.5};
	cyl.D = 0.1;
	cyl.height = 0.2;
...
t_shapes init_shapes() // инициализируем структуру, в которой будут храниться данные всех фигур
{
	t_shapes shapes;
	t_triangle	triangle;
	t_plane		plane;
...
t_objects	init_obj(t_shapes *shapes, t_camera *cam) // инициализируем структуру, в которой будем хранить расстояния (t) до каждой фигуры в данном пикселе
{
	t_objects objects;

	objects.sphere = make_sphere(&shapes->sphere, cam); // ведет рассчет расстояния до сферы, дальше до треугольника, плоскости, квадрата и цилиндра соответственно
	objects.trian = make_trian(&shapes->trian, cam); //-1 = выключение фигуры
...
t_light		init_light()
{
	...
	light.ltype = POINT; // тип освещения, пока не используется
	light.position = (t_vec3f){0.0, 2.0, 0.0};
	light.intensity = 0.4;
