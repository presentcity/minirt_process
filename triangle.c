...
static t_vec3f		triangle_normal(t_triangle *trian) // нормаль точки на поверхности треугольника
{
	return ((t_vec3f)(crossproduct(trian->edge1, trian->edge0)));
}

double	make_trian(t_triangle *triangle, t_camera *cam) // https://www.scratchapixel.com/lessons/3d-basic-rendering/ray-tracing-rendering-a-triangle/ray-triangle-intersection-geometric-solution
{
	t_vec3f tvec;
	t_vec3f qvec;
	t_vec3f pvec;
